      ...
      if (isValid || allowInDev) {
        ...
        if (user) {
          ...
          if (order.items && order.items.length > 0) {
            // Cart-based order
            courseIds = order.items.map(item => String(item.courseId).trim()).filter(Boolean);
            console.log('🛒 Cart order - courses:', courseIds);
          } else if (order.courseId) {
            // Single course order
            courseIds = [String(order.courseId).trim()].filter(Boolean);
          ...
          const normalizedExistingCourses = user.purchasedCourses.map(courseId => String(courseId).trim()).filter(Boolean);
          const currentCourses = new Set(normalizedExistingCourses);
          courseIds.forEach(courseId => currentCourses.add(courseId));
          const newCourses = courseIds.filter(courseId => !normalizedExistingCourses.includes(courseId));

          console.log('➕ New courses to add:', newCourses);
          console.log('📚 User current courses:', normalizedExistingCourses);

          if (newCourses.length > 0 || currentCourses.size > normalizedExistingCourses.length) {
