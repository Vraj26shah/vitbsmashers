...
class PaymentService {
  async createSingleCourseOrder(userId, courseId, subject, amount) {
    try {
      ...
      const gatewayType = gatewayFactory.getGatewayType();
      ...
      const paymentOrder = await gateway.createOrder(
        normalizedAmount, // Amount in paisa (₹1 = 100 paisa)
        'INR',
        `order_${order._id}`,
        {
          courseId: courseId,
  ...
  async createCartOrder(userId, items) {
    try {
      ...
      const normalizedItems = items.map(item => {
        ...
        if (!item.courseId) {
          throw new Error('Each cart item must include a courseId');
        }
        if (!Number.isFinite(normalizedAmount) || normalizedAmount <= 0) {
          throw new Error(`Invalid amount for course ${item.courseId}`);
        }

        return {
          courseId: item.courseId,
          title: item.subject || item.title || item.courseId,
          amount: normalizedAmount,
  ...
  async verifyPayment(orderId, paymentId, signature) {
    try {
      ...
      console.log(`🔍 Payment verification debug:`, {
        gatewayType,
        nodeEnv,
        orderId: orderId ? orderId.substring(0, 20) + '...' : null,
        paymentId,
